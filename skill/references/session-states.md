...
| 模型上下文窗口 | 建议 token_budget | 说明 |
|---------------|------------------|------|
| 128K | 80,000 - 100,000 | 留 28-48K 给输出 |
| 200K | 120,000 - 150,000 | 默认 120K 适用于此档 |
| 300K+ | 150,000 - 200,000 | 超大文档可提高预算 |

**原则**：budget 是 per-section 的输入 token 上限，不包括输出。budget 越大，每个 Section prompt 包含的上下文越丰富，但单次调用成本越高。
