...
- `--api-base claude-cli`：借用本机已登录的 claude CLI（headless，全套隔离），无需 API 地址和 key；`-m` 可用 sonnet/haiku/opus
- `--api-base "cmd:<命令模板>"`：接入任意 agent CLI 的 headless 模式（OpenCode/Pi/Codex/Gemini…），stdout 即响应、prompt 走 stdin；占位符 {model}/{prompt_file}/{system_file}/{user_file}
