...
```bash
# 前提：已完成第 0 步 llm config（此后命令无需再带 --api-base/-m）

# ① 注册模板：章节树由引擎从样例确定性扫描（LLM 不生成树、不写大 JSON），
#    LLM 只写 3 段小文本（抽取/风格/校验指令）
paper-derived template register-auto 样例.docx -n 任务书模板 --window 32768 --progress
...
paper-derived gen run -t <template-id> -i 资料1.docx -i 资料2.md \
  --window 32768 --compact --progress -O 交付文档.md
```
...
```bash
BIN=paper-derived
LLM=""   # 已 llm config 则留空；否则 LLM="--api-base <你的端点> -m <模型>"
