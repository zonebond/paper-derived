...
```bash
$PAPER_DERIVED_BIN template register <sample-file> -n <template-name> -d "描述"
```

拿到 prompt，执行它。你的任务是分析样例文档的结构、风格、内容模式、隐含校验规则。

## Step 2: 解析并确认

```bash
$PAPER_DERIVED_BIN template register <sample-file> -n <template-name> --parse /tmp/pd/register-response.json
```

解析后模板存入 `~/.paper-derived/templates/<name>/profile.json`。
